...
class CLIPInterface(torch.nn.Module):
    ...
    def set_frozen_embeddings(self, se):
        if se.shape == self.frozen_embeddings.shape:
            self.state_dict()['frozen_embeddings'].copy_(se)
        else:
            raise RuntimeError(f"Error: Incorrect Frozen Embedding Shape {se.shape}, Expecting {self.frozen_embeddings.shape}!")
